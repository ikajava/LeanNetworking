...
class Pin : NSObject, URLSessionDelegate {
    ...
    private static let pubKey = ["KEMk92zeeRAUvSFH9wiY3T9oaTGOjfsXG1z/z+vooBo="];
    private static let full = ["47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU="];
        ...
        if(isServerTrusted && challenge.protectionSpace.host == "staging-723489234y.albums.app") {
            ...
            if #available(iOS 10.0, *) {
                ...
                if let pubKeyData = SecKeyCopyExternalRepresentation(pubKey, &error) {
                    ...
                    if(!SessionDelegate.pubKey.contains(sha256Key)) {
                ...
                if(!SessionDelegate.full.contains(sha256Data)) {
                    isServerTrusted = false
                }
            }
        }
